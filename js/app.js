...
// rounded, lightly wobbly rectangle
function roughRect(x, y, w, h, seed, amt = 1.2, radius) {
  const r = radius ?? Math.min(10, w * 0.12, h * 0.12);
  const rr = rng(seed); const j = () => (rr() - 0.5) * 2 * amt * JITTER;
  const P = (px, py) => `${(px + j()).toFixed(1)} ${(py + j()).toFixed(1)}`;
  return `M${P(x + r, y)} L${P(x + w - r, y)} Q${x + w} ${y} ${P(x + w, y + r)} L${P(x + w, y + h - r)} Q${x + w} ${y + h} ${P(x + w - r, y + h)} ` +
    `L${P(x + r, y + h)} Q${x} ${y + h} ${P(x, y + h - r)} L${P(x, y + r)} Q${x} ${y} ${P(x + r, y)} Z`;
}
...
const HEADINGS = [
  ...
  [8.95, 'A glimpse of behaviour', 'Different ways to begin the same task'],
...
function makeLayout(mobile) {
  if (!mobile) return {
    ...
    stack14: { x: 160, y: 380, s: 1 }, glass: { x: 450, y: 310 }, findings: { x: 690, w: 250, h: 70, y0: 175, dy: 92 },
...
function buildScene(svg, L) {
  ...
  // "watercolour wash": pigment spreads (displacement) and softens (blur) as a figure dissolves
  const WASH_STEPS = 10;
  for (let i = 0; i < WASH_STEPS; i++) {
    const t = i / (WASH_STEPS - 1);
    const f = el('filter', { id: 'wash-' + i, x: '-60%', y: '-60%', width: '220%', height: '220%' }, defs);
    el('feTurbulence', { type: 'fractalNoise', baseFrequency: 0.09, numOctaves: 2, seed: 3, result: 'n' }, f);
    el('feDisplacementMap', { in: 'SourceGraphic', in2: 'n', scale: (t * 26).toFixed(1), xChannelSelector: 'R', yChannelSelector: 'G', result: 'd' }, f);
    el('feGaussianBlur', { in: 'd', stdDeviation: (t * 2.2).toFixed(2) }, f);
  }
  S.WASH_STEPS = WASH_STEPS;
  ...
  {
    const g = S.shakti;
    // imperfect outer square (hand-cut edges), four small people inside
    // organic blob: a squarish rounded shape with softly varying radius
    let outer = '';
    { const rr = rng(60), n = 24, pts = [];
      for (let i = 0; i < n; i++) {
        const a = (i / n) * Math.PI * 2;
        // superellipse-ish base (soft square) with gentle organic variation
        const base = 100 / Math.pow(Math.pow(Math.abs(Math.cos(a)), 3.2) + Math.pow(Math.abs(Math.sin(a)), 3.2), 1 / 3.2);
        const rad = base * (0.93 + rr() * 0.1) * (1 + 0.04 * Math.sin(a * 3 + 1.2));
      ...
      for (let i = 0; i < n; i++) {
        ...
        outer += (i ? '' : `M${p1[0].toFixed(1)} ${p1[1].toFixed(1)} `) + `C${c1[0].toFixed(1)} ${c1[1].toFixed(1)} ${c2[0].toFixed(1)} ${c2[1].toFixed(1)} ${p2[0].toFixed(1)} ${p2[1].toFixed(1)} `;
      }
      outer += 'Z';
    }
    el('path', { d: outer, fill: C.paleTeal, opacity: 0.9 }, g);
    el('path', { d: outer, ...PEN, 'stroke-width': 1.2, opacity: 0.45 }, g);
    const four = [[-42, -36, C.teal], [42, -36, C.sage], [-42, 46, C.terracotta], [42, 46, C.gold]];
    four.forEach(([x, y, c], i) => { const p = person(g, c, 62 + i, 1.7); setT(p, x, y, 1, (i % 2 ? 1 : -1) * 5); });
  ...
  {
    const g = S.glass;
    el('circle', { cx: 0, cy: 0, r: 38, fill: C.paleTeal, opacity: 0.35 }, g);
    el('circle', { cx: 0, cy: 0, r: 38, fill: 'none', stroke: C.teal, 'stroke-width': 2, opacity: 0.9 }, g);
    el('path', { d: 'M28 28 L52 52', stroke: C.teal, 'stroke-width': 3.5, 'stroke-linecap': 'round', opacity: 0.9 }, g);
  ...
  S.findings = FINDINGS.map(([title, desc, color], i) => {
    ...
    const t2 = TB(fText, 0, 0, desc, { 'font-size': 12, fill: C.text2 }, L.mobile ? 26 : 32);
    const rText = el('g', {}, g);
...
function makeRenderer(S, L, ctx) {
  ...
  // 10→11: 48 cohort members (8 per colour) travel to the scales and become the marks
  const morphTarget = new Array(S.cohort.length).fill(-1);
  {
    const used = [0, 0, 0, 0, 0, 0];
    S.cohort.forEach((p, i) => {
      const ui = (i * 7 + S.cohortPos[i].row) % 6; // same colour rule as the cohort
      if (used[ui] < 8) { morphTarget[i] = ui * 8 + used[ui]; used[ui]++; }
    });
  }
  const markT = S.marks.map(m => track([
    [10.86, { x: m.x, y: m.y, s: 1, o: 0 }], [10.96, { o: 1 }],
  ...
  function render(g) {
    ...
    const WASH_STEPS = S.WASH_STEPS, dimA = seg(g, 9.55, 9.8), riseB = seg(g, 9.62, 9.9), gone = seg(g, 10.55, 10.78);
    const morphT = seg(g, 10.55, 10.92), morphOut = seg(g, 10.9, 10.97), morphIn = seg(g, 10.55, 10.75);
    const SQ = [[-42, -36], [42, -36], [-42, 46], [42, 46]]; // the four small people of the Shakti mark
    S.cohort.forEach((p, i) => {
      ...
      let o = e, wash = 0;
      // 8→9: everyone except the four retained people washes away, staggered like spreading water
      const dP = seg(g, 9.0 + P.delay * 1.5, 9.22 + P.delay * 1.5);
      if (inA.has(i)) o *= lerp(1, 0.07, dimA);
      else if (inB.has(i)) { if (g < 9.62) { o *= lerp(1, 0.07, dP); wash = dP; } else o *= lerp(0.07, 1, riseB); }
      else { o *= lerp(1, 0.07, dP); wash = dP; }
      const washing = wash > 0.02 && wash < 0.98;
      p.setAttribute('filter', washing ? `url(#wash-${Math.round(wash * (WASH_STEPS - 1))})` : 'none');
      let x = lerp(ox, P.x, e), y = lerp(oy, P.y, e), sc = lerp(0.25, P.s, e) * (1 + 0.22 * Math.sin(Math.PI * wash));
      const mt = morphTarget[i];
      if (mt >= 0 && g > 10.5) { // this person morphs into a scale mark
        const m = S.marks[mt];
        x = lerp(x, m.x, morphT); y = lerp(y, m.y, morphT); sc = lerp(sc, 0.3, morphT);
        o = lerp(o, 0.9, morphIn) * (1 - morphOut);
      } else o *= 1 - gone;
      setT(p, x, y, sc, P.rot * (1 - morphT)); setO(p, o);
    });
    S.glimpseA.forEach(gg => setO(gg, seg(g, 9.18, 9.35) * (1 - seg(g, 9.55, 9.72))));
    ...
    cards.forEach((c, i) => {
      ...
      const two = true; f.t1.setAttribute('x', 30); f.t1.setAttribute('y', two ? 22 : c.h / 2 - 3); f.t2.setAttribute('x', 30); f.t2.setAttribute('y', two ? 42 : c.h / 2 + 13); Array.from(f.t2.childNodes).forEach(ts => ts.setAttribute('x', 30));
